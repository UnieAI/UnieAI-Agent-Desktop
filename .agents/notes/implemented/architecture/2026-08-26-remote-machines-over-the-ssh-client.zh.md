...
一組有條件執行的測試（`tests/live-connection.e2e.ts`）用同一份程式碼對真實 `sshd` 執行：解析、可達性與其失敗訊息、連線重用的耗時、離開狀態的傳遞、兩條輸出串流的分離、帶單引號的環境變數、存在與不存在的工作目錄，以及多位元組輸出。沒有 `DSH_SSH_TEST_CONFIG` 與 `DSH_SSH_TEST_ALIAS` 時它回報自己被略過，而不是空洞地通過；檔案本身寫明它期待的那台用完即丟的伺服器怎麼架。

## Deferred

- **執行世界的 adapter。** `subprocess-ssh` 與 `fs-ssh` 才是讓 bash、終端機、language server 與檔案工具真的跑在那台機器上的套件；這份 note 交付的是它們共用的擁有者。
- **遠端行程的生命週期。** 遠端指令會活得比啟動它的連線久：對真實伺服器實測，殺掉本機客戶端之後，遠端行程仍在執行——有沒有配置終端機都一樣。subprocess adapter 必須擁有遠端 process group，就像 E2B adapter 那樣；擁有者刻意不保留任何逐指令的遠端狀態。
