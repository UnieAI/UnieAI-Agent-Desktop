...
- `dsh-workspace` canonicalizes and validates workspace directories through `ctx.fs`, so a workspace is a directory in whatever world is mounted. The filesystem is a mandatory injection there: a registry that canonicalized against the host while the tools worked on another machine would make every remote directory unreachable while looking correct locally.
