...
interface WorktreeWindowProps {
  ...
  /**
   * UnieAI fork divergence: rendered inside the shell's right column instead
   * of floating over the conversation. A docked window fills its container, so
   * it carries no position, no drag and no resize — the column owns its width.
   */
...
export function WorktreeWindow(props: WorktreeWindowProps): React.ReactElement {
  ...
  const className = [
    'uvf_win',
    docked ? 'uvf_win_docked' : '',
    folded ? 'uvf_win_folded' : '',
    maximized && !docked ? 'uvf_win_max' : '',
  ].filter(Boolean).join(' ')
  // A docked window fills its column: the geometry belongs to the column, so
  // stating a position here would fight it.
  ...
  return <section className={className} style={style} data-interaction={interaction ?? undefined} aria-label={`${title} · ${basename(props.file)}`}>
    <header className="uvf_windowHeader" onPointerDown={docked ? undefined : onHeaderPointerDown} onDoubleClick={docked ? undefined : onHeaderDoubleClick}>
    ...
    {!docked && !folded && !maximized ? RESIZE_DIRECTIONS.map((direction) => <span
      key={direction}
