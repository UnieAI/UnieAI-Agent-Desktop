 ...
 */
import type { InputTriggerSource } from '@unieai/uad-client-ui-input-trigger/client'
...
export function createPanelSlashSource(sendInstruction: (sessionId: SessionId, instruction: string) => void): InputTriggerSource {
  return {
    ...
    name: 'genui',
    order: 60,
    // FORK DIVERGENCE (vendor/genui/patch/): filter on the query. Every other
    // `InputTriggerSource` ends in `fuzzyCandidates(rows, req.query)`; upstream
    // returns this candidate for EVERY query, so `panel` was listed beside
    // whatever the person typed and could take the Enter — typing `/goal` and
    // pressing Enter completed `/panel`.
    candidates: async (_session, req) => {
      const query = req.query.trim().toLowerCase()
      return query === '' || 'panel'.startsWith(query)
        ? [{
          name: 'panel',
