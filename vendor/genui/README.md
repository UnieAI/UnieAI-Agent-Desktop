> **Vendored copy.** This is `@changfenhuang/dsh-genui` 0.9.6 (MIT, https://github.com/omdsh-dev/dsh-genui), pinned as source and republished as `@unieai/genui` with its imports rescoped to this fork's package names. Upstream's text follows unchanged; the divergences are logged in [vendor/README.md](../README.md).
...
dsh plugin --profile web add @unieai/genui
...
If you installed from `github:omdsh-dev/dsh-genui` before v0.9.2, pnpm may keep the dependency under the old `@omdsh-dev/dsh-genui` key even though the repository now declares `@unieai/genui`. The loader resolves plugins from the profile's dependency keys, so a later reinstall can then fail with `Cannot find package '@unieai/genui'`. Re-add the plugin under its current package name:
...
- **Stuck on git credentials / 404 during install?** The repository and npm package are public and require no login. Run `npm view @unieai/genui version` to verify the package name and public registry; if a newly published version still returns 404, retry shortly or use the GitHub install command above in the meantime.
- **Installed but scene3d/mermaid/echarts don't render?** The engines (mermaid / three / echarts) are no longer inlined in client.js — they load on demand the first time they're used (`/plugins/@unieai/genui/assets/*.js`, hosted by the plugin's own HTTP routes). First restart dsh web + hard refresh (Cmd+Shift+R); still broken, remove and reinstall (`dsh plugin --profile web remove @unieai/genui`, then add again). Hosts without the asset routes degrade to source/load-error hints — update dsh.
