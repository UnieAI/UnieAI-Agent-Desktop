...
These three **default-off reference configurations** connect one memory system to DSH through [`@unieai/uad-mcp-client`](../../packages/mcp/mcp-client/README.md). Pick one, or copy the same generic MCP row for another server.
...
```sh
rabi web --patch "$PWD/examples/mcp-memory/memorix.cordis.yml"
```
...
rabi web --patch "$PWD/examples/mcp-memory/mcp-reference-memory.cordis.yml"
```
...
rabi web --patch "$PWD/examples/mcp-memory/engram.cordis.yml"
```
