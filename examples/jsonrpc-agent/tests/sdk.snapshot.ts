 ...
 * REAL `@unieai/uad-sdk-client`, drives one turn over stdio JSON-RPC,
...
import { resolveExampleLaunch } from '@unieai/uad-loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@unieai/uad-sdk-client'
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@unieai/uad-system-prompt') return []
