...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__DSH_BOOT__. '
  + 'From a repository checkout, run `pnpm rabi web`; an installed package uses `rabi web`. '
  + 'For client-plugin HMR, run `pnpm rabi web` together with `pnpm run dev:web`.'
...
function clientDocumentTitle(): Plugin {
  ...
  return {
    ...
    transformIndexHtml(html) {
      // Replace whatever title index.html carries. Matching a literal
      // placeholder silently did nothing once that file was rebranded, so a
      // build that set DSH_CLIENT_TITLE could not change the tab.
