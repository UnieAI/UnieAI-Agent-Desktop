...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  // The UnieAI mark, not the upstream harness wordmark: `web-app` composes
  // `ui-brand-unieai` and deliberately leaves the official-brand row out.
  expect(document.querySelector('svg[viewBox="0 0 499.28 444.55"]')).not.toBeNull()
  ...
  for (const plugin of ['@unieai/uad-client-ui-layout', '@unieai/uad-client-ui-sidebar', '@unieai/uad-client-ui-conversation', '@unieai/uad-client-ui-tool']) {
