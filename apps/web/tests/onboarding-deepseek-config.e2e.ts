...
// credential is absent, the welcome notice is the only step that takes the
// screen, and a key written on the Models page lands in an isolated harness
// home without a reload or model call.
...
describe.skipIf(MODE === 'record')('web e2e: first-run welcome notice and DeepSeek credential setup', () => {
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    ...
    // Nothing else takes the screen: Rabi provisions models from the signed-in
    // account, so a user who wants a key of their own enters it on the Models
    // page's own first-run setup card instead of being stopped for one.
    expect(await page.locator('#root').evaluate(root => (root as HTMLElement).inert)).toBe(false)
    expect(await page.getByRole('dialog').count()).toBe(0)

    await page.getByRole('button', { name: '设置', exact: true }).click()
    const setup = page.getByRole('dialog', { name: '设置' })
    await setup.waitFor({ timeout: 10_000 })
    await setup.getByRole('button', { name: '模型' }).click()
    const keyInput = setup.getByRole('textbox', { name: 'API 密钥', exact: true })
    ...
    // The ordinary Models row reuses the refreshed join and exposes the
    // configured write-only placeholder without a reload.
    const deepSeekRow = setup.getByText('DeepSeek', { exact: true }).first()
    await deepSeekRow.waitFor({ timeout: 10_000 })
    await deepSeekRow.locator('xpath=ancestor::li').getByRole('button', { name: '编辑' }).click()
    const configuredInput = setup.getByLabel('API 密钥', { exact: true })
  ...
  it('never paints the takeover chrome on a configured reload, even with the settings join held open', async () => {
    ...
    // inside its visible branch, so the deciding window paints and blocks
    ...
    // The title comes from the mirrored copy rather than a literal, so a copy
    // revision cannot quietly turn this sampler into one that watches nothing.
    await page.addInitScript((title: string) => {
      const sightings: string[] = []
      ;(window as unknown as { __takeoverSightings: string[] }).__takeoverSightings = sightings
      setInterval(() => {
        if (document.querySelector(`[role="dialog"][aria-label="${title}"]`) !== null) {
  ...
  it('configures arbitrary DeepSeek models and prompts after the selected model is removed', async () => {
    ...
    // The picker opens straight onto the model list; there is no submenu step.
