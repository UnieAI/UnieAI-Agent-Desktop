// Real-host smoke: spawn `uad web` with a real key, walk the full flow
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`uad web not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /uad web: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`uad web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `uad web` serves every production manifest plugin; until every UI
...
describe('uad web keyless CLI smoke', () => {
