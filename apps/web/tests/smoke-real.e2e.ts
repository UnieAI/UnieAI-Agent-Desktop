// Real-host smoke: spawn `rabi web` with a real key, walk the full flow
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`rabi web not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /rabi web: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`rabi web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `rabi web` serves every production manifest plugin; until every UI
...
describe('rabi web keyless CLI smoke', () => {
