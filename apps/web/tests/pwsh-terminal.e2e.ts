...
import { resolvePwshPath } from '@unieai/uad-pwsh-local'
import {
  assertFixtureInventory, captureStableAria, compareOrRefreshGolden,
  fixtureUserPrompts, launchWebScaffold, seedSession, webSnapshotMode,
  type WebScaffold,
} from './scaffold.ts'
import { connectFreshWorkspace, newEnglishPage, saveFailureShot, viewToggle } from './support.ts'
...
// The overlay swaps the shipped bash executor for @unieai/uad-pwsh-local;
...
describe.skipIf(MODE === 'record' || !HAS_PWSH)('web e2e: pwsh calls use the bash terminal-card layout', () => {
  ...
  it('renders the seeded pwsh call as a terminal card with the parsed exit pill', async () => {
    ...
    await result.click()
    await viewToggle(page).waitFor({ timeout: 15_000 })
