...
the deliberate composition divergences from `rabi web` — are documented in
...
## Do not import `@unieai/uad-client-*` here
...
`@unieai/uad-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@unieai/uad-client-web` and the boot-manifest
type from `@unieai/uad-client-modules/client`: booting the real shell is what
...
`@unieai/uad-client-runtime/client` because `client/runtime` is reachable
