...
import { entryListSchema } from '@unieai/cordis-plugin-include'
import { evaluate } from '@unieai/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@unieai/uad-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@unieai/uad-base', '@unieai/uad-web-app'])
    ...
    for (const name of ['@unieai/uad-pwsh-sandbox', '@unieai/uad-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@unieai/uad-base'])
