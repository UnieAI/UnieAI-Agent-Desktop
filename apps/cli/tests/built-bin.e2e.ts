...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('uad --profile web')
    expect(help.stdout).toContain('uad plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    ...
    try {
      ...
      expect(web.stdout).toContain('Usage: uad --profile web')
      expect(web.stdout).toContain('--port <port>')
      expect(web.stdout).not.toContain('uad web: http://')
      ...
      expect(wildcardHost.stderr).not.toContain('uad web: http://')
      ...
      expect(headlessHelp.stdout).toContain('Usage: uad --profile headless')
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    ...
    try {
      ...
      expect(result.stderr).toContain('uad plugin --profile nope add')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    // `uad plugin --profile x add .` from a plugin checkout must install THAT
