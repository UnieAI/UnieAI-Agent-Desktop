/** Assembled keyless snapshot for the default `rabi web` browser handoff. */
...
const openingMessage = 'rabi web: opening the default browser; pass --no-open to disable'
...
if (process.env.DSH_EXAMPLE_MODE === 'lib' && !builtArtifactsExist) {
  throw new Error('rabi web browser-open snapshot requires built CLI and Web artifacts in lib mode')
}
...
describe.skipIf(!builtArtifactsExist)('rabi web browser-open assembled snapshot', () => {
  it('hands the reachable page to the default browser after the shipped tree settles', async () => {
    ...
    const readyUrl = /rabi web: (http:\/\/[^\s]+)/u.exec(result.stdout)?.[1]
    ...
    if (readyUrl === undefined || openLine === undefined || !opening) {
      throw new Error(`rabi web browser-open evidence missing\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
  ...
  it('rejects a project browser command before starting the Web app', async () => {
    ...
    expect({
      ...
      opening: result.stdout.includes(openingMessage),
      opened: result.stdout.includes('dsh browser-open: '),
      ready: result.stdout.includes('rabi web: '),
    }).toMatchInlineSnapshot(`
      {
