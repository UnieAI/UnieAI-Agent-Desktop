...
The open-source Chromium build for **Windows on x86-64 (and on arm64, which runs it under emulation)**, carried so the operator browser works on a machine that has no browser of its own.

Chromium snapshot revision: `1685180` — BSD-3-Clause, and the LICENSE beside the payload is the text it is published under.
