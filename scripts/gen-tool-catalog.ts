...
import GoalService from '@unieai/uad-goal'
import SystemPrompt from '@unieai/uad-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@unieai/uad-tools'
import LocalBashExecutor from '@unieai/uad-bash-local'
import * as BashEnvPlugin from '@unieai/uad-shell-env'
import { PwshLocalExecutor } from '@unieai/uad-pwsh-local'
...
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@unieai/uad-attachment'
import UserQuestionService from '@unieai/uad-user-questions'
import PlanModeController from '@unieai/uad-plan-mode'
import WebRuntime from '@unieai/uad-web'
import * as WebSearchExa from '@unieai/uad-web-search-exa'
import * as WebFetchLocal from '@unieai/uad-web-fetch-http'
import SubagentRuntime from '@unieai/uad-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@unieai/uad-subagent'
import * as ToolSubagentControl from '@unieai/uad-tool-subagent-control'
import * as ToolSubagentListAgents from '@unieai/uad-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@unieai/uad-tool-subagent-report'
...
import * as ToolAskUser from '@unieai/uad-tool-ask-user'
import * as ToolBash from '@unieai/uad-tool-bash'
import * as ToolPwsh from '@unieai/uad-tool-pwsh'
import * as ToolBashPersistent from '@unieai/uad-tool-bash-persistent'
import * as ToolPwshPersistent from '@unieai/uad-tool-pwsh-persistent'
import CordisHostRunner from '@unieai/uad-cordis-host-runner'
import * as ToolCordis from '@unieai/uad-tool-cordis'
import * as ToolFs from '@unieai/uad-tool-fs'
import * as ToolFsSearch from '@unieai/uad-tool-fs-search'
import * as ToolStrReplaceEditor from '@unieai/uad-tool-str-replace-editor'
import TerminalSessionService from '@unieai/uad-terminal'
import * as ToolPty from '@unieai/uad-tool-terminal'
import * as ToolGoal from '@unieai/uad-tool-goal'
import * as ToolSchedule from '@unieai/uad-schedule'
import Lsp from '@unieai/uad-lsp'
import * as ToolLsp from '@unieai/uad-tool-lsp'
import * as ToolSkill from '@unieai/uad-tool-skill'
import * as ToolSessionQuery from '@unieai/uad-tool-session-query'
import * as ToolTasks from '@unieai/uad-tool-jobs'
import type TeamService from '@unieai/uad-experimental-agent-team'
import * as ToolTeam from '@unieai/uad-experimental-tool-agent-team'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@unieai/uad-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@unieai/uad-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@unieai/uad-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `DSH_*` environment comes from `@unieai/uad-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@unieai/uad-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@unieai/uad-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@unieai/uad-tool-bash-persistent',
  ...
  {
    pkg: '@unieai/uad-tool-pwsh-persistent',
  ...
  {
    pkg: '@unieai/uad-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@unieai/uad-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. The image tool is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@unieai/uad-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@unieai/uad-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@unieai/uad-tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@unieai/uad-tool-session-query',
  ...
  {
    pkg: '@unieai/uad-tool-subagent-control',
  ...
  {
    pkg: '@unieai/uad-tool-subagent-report',
  ...
  {
    pkg: '@unieai/uad-experimental-tool-agent-team',
    dir: 'tool-agent-team',
