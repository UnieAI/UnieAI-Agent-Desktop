...
function checkManifest(
  ...
  if (owner.packageName === '@unieai/uad-invariants') return
  if (manifest.peerDependencies?.['@unieai/uad-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@unieai/uad-invariants must be a workspace:^ peerDependency',
    )
  }
  if (manifest.devDependencies?.['@unieai/uad-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@unieai/uad-invariants must also be a workspace:^ devDependency',
    )
  }
}
...
function checkBuild(
  ...
  if (owner.packageName !== '@unieai/uad-invariants'
