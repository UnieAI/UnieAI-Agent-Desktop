...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-studio-sources': { kind: 'none', reason: 'Re-reads a tool result the model already produced and shows a person where it came from. It registers no tool, prompt, schema, or context, and nothing it renders reaches a model request.' },
  'packages/bundle/remote-machine': { kind: 'none', reason: 'A composition patch that registers nothing; it changes which machine the tools work on, and each of those tools documents its own model-facing contract.' },
  'packages/ssh/fs-ssh': { kind: 'indirect', reason: 'The provider changes which machine the file tools built on ctx.fs read and write; those tools own their model-facing contracts, and nothing here registers a prompt, schema, or tool.' },
  'packages/ssh/subprocess-ssh': { kind: 'indirect', reason: 'The provider changes where every tool built on ctx.subprocess does its work; those tools own their model-facing contracts, and nothing here registers a prompt, schema, or tool.' },
  'packages/client/ui-machines': { kind: 'none', reason: 'Shows a person which machine their work runs on and lets them change it; it registers no tool, prompt, schema, or context, and the model is never told which machine it is on.' },
  'packages/machines/machines': { kind: 'none', reason: 'Answers which machines exist and which one is current for a person\'s surfaces; it registers no tool, prompt, schema, or context, and the model is told neither.' },
  'packages/machines/machine-metrics': { kind: 'none', reason: 'Samples what a machine is doing for a person\'s surface to draw; it registers no tool, prompt, schema, or context, and no reading reaches a model request.' },
  'packages/client/ui-machine-gauges': { kind: 'none', reason: 'Draws the machine reading the host sampled; it registers no tool, prompt, schema, or context, and the model is never told what the machine is doing.' },
  'packages/machines/execution-router': { kind: 'indirect', reason: 'The routed providers change which machine the tools built on ctx.fs and ctx.subprocess work on; those tools own their model-facing contracts.' },
  'packages/ssh/ssh': { kind: 'none', reason: 'Answers which machines exist and composes ssh invocations for the execution-world adapters; it registers no tool, prompt, schema, or context, and the model never learns an alias from here.' },
  'packages/client/ui-pet': { kind: 'none', reason: 'A drawing in the corner of a window, driven by the client session snapshot a person is already looking at. It registers no tool, prompt, schema, or context, and the model is never told it exists.' },
  ...
  'packages/client/ui-brand-unieai': { kind: 'none', reason: 'Browser-side presentation occupants; registers nothing model-facing.' },
  'packages/unieai/web-gate': { kind: 'none', reason: 'Gates browser traffic and serves a sign-in document; registers nothing model-facing.' },
  'packages/client/ui-unieai-account': { kind: 'none', reason: 'Browser-side account settings surface; the desktop account gateway it reads owns every request and nothing here reaches a model.' },
  'packages/client/ui-unieai-providers': { kind: 'none', reason: 'Browser-side API Provider settings surface; the providers it lists belong to the web product and nothing here reaches a model.' },
  ...
  'packages/llm/llm-unieai-cloud': { kind: 'indirect', reason: 'The route registers a dsh-llm-pi-ai adapter; that adapter owns the assembled provider request and every model-visible field in it.' },
  'packages/unieai/mcp-supervisor': { kind: 'indirect', reason: 'The supervisor mounts dsh-mcp-client instances; that package owns every discovered tool\'s name, schema, and result.' },
  ...
  'packages/util/upstream-names': { kind: 'none', reason: 'Two pure functions mapping package names for module resolution; registers nothing model-facing.' },
