...
const MERGE_HEAD = /declare module ['"](?:@unieai\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@unieai/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@unieai/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@unieai/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@unieai/cordis'` Context merge
...
/**
 * Every event name a `declare module '@unieai/cordis'` Events merge declares in one
