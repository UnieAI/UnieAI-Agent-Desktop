...
const SESSION_PACKAGE = '@unieai/uad-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@unieai/uad-session/types'
 ...
 * top-level declaration (in `@unieai/uad-session`) and any declaration
 * merge inside a `declare module '@unieai/uad-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @unieai/uad-session/known-event-types',
    ' */',
    '',
    '/**',
