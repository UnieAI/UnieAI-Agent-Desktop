...
describe('DSH package license gate', () => {
  it('checks root, the named CLI, and prefixed package names while ignoring other families', () => {
    const root = createWorkspace()
    writeManifest(root, 'apps/cli/package.json', { name: '@unieai/rabi', license: 'MIT' })
    ...
    expect(inspectDshPackageLicenses(root)).toEqual({
      packageCount: 3,
      failures: [
        'packages/core/agent/package.json: @unieai/uad-agent must declare "license": "MIT"; found "BSD-3-Clause".',
      ],
    })
  })
  ...
  it('rejects a missing license declaration', () => {
    const root = createWorkspace()
    writeManifest(root, 'packages/core/agent/package.json', { name: '@unieai/uad-agent' })

    expect(inspectDshPackageLicenses(root).failures).toEqual([
      'packages/core/agent/package.json: @unieai/uad-agent must declare "license": "MIT"; found undefined.',
    ])
  })
})
