...
| `ctx.connectors` | `seam` | [`connector`](../packages/connector/connector) | - | - | - | Which external services this harness has access to, and a token that is valid right now. It owns no protocol: the human conversation is the authorization seam's, the durable grant is a credentials GrantRecord, and what a connector is for belongs to whatever registers tools against it. |
...
| `ctx.machineMetrics` | `core` | `machines/machine-metrics` | - | `apiproxy` | - | Samples one machine by running a guarded shell command through `ctx.subprocess`, which the router already aims at whichever machine is current — so a remote reading and a local one are one code path and this service never learns machines exist. Every field it cannot read is absent rather than zero, and a processor percentage is the difference between consecutive samples. |
