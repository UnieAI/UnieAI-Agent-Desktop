...
import type { PreToolDecision, ToolExecution } from '@unieai/uad-tools'
...
Runnable leaves load their plugin trees from `examples/*/cordis.yml`; the root `demo:*` scripts and those leaf directories are the authoritative inventory. The product `rabi` launcher owns Web and one-shot headless execution, ACP leaves use [`@unieai/uad-acp-demo`](../../packages/examples/acp-demo), and JSON-RPC leaves use [`@unieai/uad-sdk-jsonrpc-demo`](../../packages/examples/jsonrpc-demo). The headless snapshot leaf mounts [`@unieai/uad-agent-spine-demo`](../../packages/examples/agent-spine-demo) and JSONL persistence explicitly, then drives them through an example-owned test fixture rather than a shipped app package.
...
| Plan mode | [`@unieai/uad-plan-mode`](../../packages/plan/plan-mode/README.md) — logged `plan/mode` state, the `plan:policy` guidance section, `/plan [message]` entry, `/plan off` direct exit, and the user-reviewed `exit_plan_mode` exit; enforcement stays on the independent sandbox/approval axes |
