...
A running `rabi` is a plugin tree composed at boot from ordered layers.
...
Each declares itself in its own `package.json` under a `rabi` field: `dsh.profile` lists a profile's bundles, and `dsh.bundle` points at a bundle's patch file.
