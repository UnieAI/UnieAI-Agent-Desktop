...
flowchart LR
  ...
  svc_connectors["ctx.connectors<br/>Connector seam"]
  ...
  pkg_ssh["ssh"]
  svc_ssh["ctx.ssh<br/>Remote machine book and shared connection owner"]
  ...
  pkg_terminal_operator --> svc_operatorTerminals
...
| `ctx.connectors` | `seam` | [`connector`](../packages/connector/connector) | - | - | - | 這個 harness 有哪些外部服務的存取權，以及一個此刻有效的權杖。它不擁有任何協定：跟人的對話屬於 authorization 接縫，持久的授權是 credentials 的 GrantRecord，而一個連接器是為了什麼，屬於任何對著它註冊工具的東西。 |
