...
`pnpm rabi web` 會印出它綁定的網址並開啟瀏覽器。加 `--no-open` 可以留在終端機，加 `--port <n>` 可以指定連接埠，預設是 3080。
...
`apps/desktop` 是一個 Electron 視窗，罩在它自己啟動的 harness 上。它不加任何產品行為：它在一個由作業系統指派的 loopback 連接埠上啟動 `rabi web`，等那個伺服器印出網址那一行，然後載入它。
