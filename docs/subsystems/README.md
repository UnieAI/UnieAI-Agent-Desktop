...
| [operator-browser.md](operator-browser.md) | the browser a person drives: `OperatorBrowserId`/`View`/`OpenSpec`, the pointer and key gesture vocabularies, `OperatorBrowserError`, and the loopback fence |
...
| [model-vision.md](model-vision.md) | what a text model does about what it cannot see: the one-shot `page_screenshot` browser and the `image_inspect` delegation to a configured vision route |
