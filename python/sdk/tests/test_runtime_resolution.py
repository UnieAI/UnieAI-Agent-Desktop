...
def test_default_config_is_shipped_with_the_package() -> None:
    ...
    assert "@unieai/uad-agent-spine-demo" in config
    assert "@unieai/uad-session-persistence-jsonl" in config
    assert "@unieai/uad-session-checkpoint-policy" in config
