...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@unieai/uad-llm'
import type { GenerateOptions, StreamChunk } from '@unieai/uad-llm'
import SessionStore, { SessionId } from '@unieai/uad-session'
import SessionTitleService from '@unieai/uad-session-title'
import * as providerPlugin from '@unieai/uad-session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    ...
    "- name: '@unieai/uad-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@unieai/uad-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@unieai/uad-session-title', SessionTitleService],
    ['@unieai/uad-session-title-first-prompt-llm', providerPlugin],
  ])
