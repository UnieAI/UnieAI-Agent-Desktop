...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@unieai/uad-tool-call-timeout-policy` wrapper.
