 ...
 * execution pipeline.
 * @module @unieai/uad-tools
 */

import { Context, Service } from '@unieai/cordis'
import z from '@unieai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@unieai/uad-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@unieai/uad-scope'
import type { CallId, ContentBlock, ToolSchema } from '@unieai/uad-llm'
import { assertNever, deepFreeze, HarnessError } from '@unieai/uad-llm'
import type { Agent } from '@unieai/uad-agent'
import { snapshotJsonValue } from '@unieai/uad-session'
import type { JsonValue, UserMessage } from '@unieai/uad-session'
import type { ToolProviderResult } from '@unieai/uad-system-prompt'
import type { CodeRuntime } from '@unieai/uad-code-runtime'
...
export type { JsonValue } from '@unieai/uad-session'
...
// lives in its own UI-facing module; re-export it so `@unieai/uad-tools`
...
declare module '@unieai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@unieai/uad-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@unieai/uad-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@unieai/uad-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@unieai/uad-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@unieai/uad-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @unieai/uad-code-runtime-worker-thread) or set tools mode to "native"`)
    }
