import { createAssistantMessage, createUserMessage } from '@unieai/uad-llm'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { Context, type Fiber } from '@unieai/cordis'
import { DatabaseSync } from 'node:sqlite'
import { chmod, mkdtemp, rm, stat, writeFile } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { dirname, join } from 'node:path'
import SessionStore, { SESSION_FORMAT_VERSION, SessionId } from '@unieai/uad-session'
import type { SessionEvent, SessionHeader, SessionId as SessionIdType } from '@unieai/uad-session'
import SessionPersistence, { SessionPersistenceRevision } from '@unieai/uad-session-persistence'
import type { SessionPersistenceSnapshot } from '@unieai/uad-session-persistence'
import SqliteSessionPersistence from '@unieai/uad-session-persistence-sqlite'
import SqliteSessionQueryEngine, {
  SESSION_QUERY_SQLITE_SCHEMA_VERSION,
} from '@unieai/uad-session-query-sqlite'
import {
