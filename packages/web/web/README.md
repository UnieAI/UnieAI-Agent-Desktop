...
| Package | Role |
|---|---|
| `@unieai/uad-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@unieai/uad-web-search-exa` | Search provider: Exa |
| `@unieai/uad-web-search-perplexity` | Search provider: Perplexity |
| `@unieai/uad-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@unieai/uad-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
