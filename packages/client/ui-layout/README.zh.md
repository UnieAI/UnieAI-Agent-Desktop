...
首次 apply 时还会把页面自带的 `meta[name="theme-color"][data-boot-theme-color]` 取出，dispose 时再放回。那些是启动前的答案，而且带配色方案条件：文档中较早且 media 命中的 meta 会胜过呈现器较晚且无条件的那一个，所以只要它们还在，浏览器外框跟的就是操作系统而不是用户选的主题。页面没有标记的 `theme-color` 不会被动到。
...
框架元素还把自己**实际渲染**的侧边栏宽度以内联自定义属性 `--dsh-shell-sidebar-width`（`SIDEBAR_WIDTH_PROPERTY`）发布出去，供所有后代继承。`shell.overlay` 横跨整个应用盒子，所以这是覆盖层占位者对齐到某一栏（而不是对齐到整个框架）的唯一方式——需要让导航栏露出来的表面按它让开自己的左边缘，而这个值已经把拖动、折叠和窄视口自动折叠都算进去了。它是几何量而不是主题 token，所以任何 token 样式表都不声明它。
