...
import type { HostDescription } from '@unieai/uad-client-connection/client'
import type {
  ChatConversationViewNode, ChatSnapshot, ConversationNode, RunningToolCall, SessionId,
} from '@unieai/uad-client-runtime/client'
import type { SessionProviderComponent, TranslateNS } from '@unieai/uad-client-ui-slots'
import type { DetailsSlotProps, DetailsToolOwnerProps } from '@unieai/uad-client-ui-conversation/src/client/contract/slots.ts'
...
export function renderToolDetails(
  ...
  return (key, owner) => {
    // The panel renders two seats around one call — the tool body and the
    // annotation list beneath it — through the same callback. A double that
    // answered both would draw this package's card twice, and every
    // assertion that a card appears once would fail on its own duplicate.
