...
import { apply as settingsApply, inject as settingsInject } from '@unieai/uad-client-ui-settings/client'
import { TestRemote } from '@unieai/uad-client-test-runtime'
import {
  apply, inject, SETTINGS_NS,
} from '@unieai/uad-client-locale/client'
import type { LanguageRowInjected, LocaleRuntime } from '@unieai/uad-client-locale/client'
...
describe('locale apply', () => {
  ...
  it('provides the service with base + settings dictionaries and registers the row (declaration before or after apply)', async () => {
    ...
    expect(() => locale.register('common', 'zh-CN', {})).toThrow('already has locale')
  ...
  it('loads and refreshes the explicit Host preference after nonblocking activation', async () => {
    ...
    b.setHostPreference('zh-CN')
    b.ctx.remote.$dispatch('settings/document-updated', [LOCALE_SETTINGS_NAMESPACE, 0])
    declareItems(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const locale = b.ctx.get('locale') as LocaleRuntime
    await vi.waitFor(() => { expect(locale.getLocale().active).toBe('zh-CN') })
