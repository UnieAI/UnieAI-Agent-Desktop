...
One `settings.models.account` entry — **API Provider Settings** — showing the UnieAI account's OpenAI-compatible providers exactly as the UnieAI Copilot web product's own page of that name shows them, plus the Add Provider control that puts a new one into the same list and the per-row edit and delete that change one already in it.

It renders INSIDE the Models page, above that page's own rows, rather than as a settings section of its own. Two sections that both said "add a provider" hid the difference that matters: a row here keeps its key on the product's server, is metered by the account, and follows the person to every client signed into it; a row under the Models page's own heading keeps its key on this machine, calls the endpoint directly, and is metered by nobody. One page can name that; two pages could only imply it.
...
- **No model-catalogue sync from this page.** A provider added here does reach the composer's model picker: [`dsh-llm-unieai-cloud`](../../llm/llm-unieai-cloud/README.md) registers one `unieai` route whose catalogue is the account's entitled models and whose requests go to the product's metered relay. What this page cannot do is change which models a provider offers — see the next entry.
