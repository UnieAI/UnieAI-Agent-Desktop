...
宿主不会提供一个它无法认证的 provider，因此空选单是本插件如实渲染的一种真实状态，而非错误：`session.models` 会剔除适配器报告没有凭据的路由，选单随之显示 `没有可用的模型。`。于是一台没有存下密钥的机器读起来是一致的——选择器什么都不提供，而 Models 设置分节的首次运行凭据步骤才是索取密钥的地方。本插件刻意不再添加第二份指向那里的空状态文案：那会成为对同一个问题的第二种回答，且目的地一挪动就立刻过时。
...
- **空选单不会说明自己为何为空**——「没有配置任何 provider」与「所有已配置的 provider 都被滤掉了」，抵达读者时是同一句 `没有可用的模型。`，两者都没有点名 Models 分节才是解法。宿主把剔除表达为缺席而非一条失败行，因此本插件手上没有可用来区分两者的东西；要给它一个，得让目录携带原因，而不是让本插件去猜。
