# @unieai/uad-client-ui-model-selection
...
The Host does not offer a provider it cannot authenticate, so an empty menu is a real state this plugin renders rather than an error: `session.models` drops a route whose adapter reports no credential, and the menu says `No models available.` A machine with no key stored therefore reads consistently — the picker offers nothing, and the Models settings section's first-run credential step is the place that asks for one. This plugin deliberately adds no second empty-state copy pointing there: it would be a second answer to a question that surface already answers, and it would go stale the moment the destination moves.
...
- **The empty menu does not say why it is empty** — "no provider is configured" and "every configured provider was filtered out" reach the reader as the same `No models available.` line, and neither names the Models section as the fix. The Host reports the drop as an absence rather than as a failure row, so this plugin has nothing to distinguish them with; giving it one means the catalog carrying a reason, not this plugin guessing.
