...
describe('BootPage', () => {
  it('draws the loading skeleton before any plugin state arrives', () => {
    ...
    expect(el.textContent).toContain('UNIEAI')
  ...
  it('lists failed entries', () => {
    const { el, page } = mount()
    page.setState('@unieai/uad-client-ui-layout', 'failed')
    page.setState('ok', 'active')
    page.setState('@unieai/uad-client-ui-tool', 'failed')
    expect(el.textContent).toContain('@unieai/uad-client-ui-layout')
    expect(el.textContent).toContain('@unieai/uad-client-ui-tool')
