...
`refresh` is optional and is what keeps a figure from being a memory. The account is read once when the application starts and republished after a write this gateway performed; everything else about it moves elsewhere — a turn spends an allowance, a plan changes on the web product, another device signs out — so a page drawn from the start-up reading presents whatever was true at launch as if it were current. **The Usage page asks when it opens and every minute it stays open, and stops when it closes**: an allowance is exactly the figure that moves while nobody is looking, and a product call spent on a page nobody is on serves nobody. The Account page asks once on open, because a plan name and an avatar change when a person changes them.
