...
export const zh = {
  ...
  failed: '挂载失败',
  unloading: '卸载中',
  note: '本目录只负责列出。安装插件请使用 dsh plugin --profile web add <包名>；启用或停用请改写该 profile 的 cordis.patch.yml。',
...
export const zhTW = {
  ...
  failed: '掛載失敗',
  unloading: '卸載中',
  note: '本目錄只負責列出。安裝外掛程式請使用 dsh plugin --profile web add <套件名>；啟用或停用請改寫該 profile 的 cordis.patch.yml。',
...
export const ja = {
  ...
  failed: 'マウント失敗',
  unloading: 'アンロード中',
  note: 'このカタログは一覧するだけです。プラグインの追加は dsh plugin --profile web add <パッケージ名>、有効・無効の切り替えはプロファイルの cordis.patch.yml で行います。',
} satisfies Record<PluginInventoryLocaleKey, string>
...
export const en = {
  ...
  failed: 'Mount failed',
  unloading: 'Unloading',
  note: 'This catalogue only lists. Add a plugin to the profile with dsh plugin --profile web add <package>, and turn one on or off in that profile’s cordis.patch.yml.',
} satisfies Record<PluginInventoryLocaleKey, string>
