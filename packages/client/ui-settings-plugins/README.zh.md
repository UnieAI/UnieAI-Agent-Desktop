...
[插件页面](../ui-plugins-page/README.zh.md)的 **部署插件** 区块及其**插件配置**标签页。该区块拥有标题与紧凑的标签栏；功能插件通过 `settings.plugins.tab` 贡献页面。本包自己的标签页为每个配置由用户拥有的 Host 插件展示一张可展开卡片。卡片展示插件名称及其管辖范围；就地展开后是绑定到该插件 settings 命名空间的手写控件，每个字段标注用户是否覆盖过它，并提供重置回部署组装值的入口。

## 它在哪里

这是 cordis 插件注册表——对本部署而言「插件」是什么。它过去是从侧边栏「插件」行进入的 `settings.section`，而那一行承诺的是账号可安装的插件。现在它占据 `plugins.page.area`，插件页面上同一种形状的座位，与账号的 Studio MCP 服务器并列。区块内部没有任何改变：同一个组件、同样的标签页、同样的卡片、同一个 settings scope。没有 [`ui-plugins-page`](../ui-plugins-page/README.zh.md) 的组装无处渲染它。
