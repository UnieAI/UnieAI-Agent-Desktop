...
- **A machine is listed because it is configured, not because it answers.** Nothing is probed when the menu OPENS, so a machine that is off looks exactly like one that is on until it is picked. Picking one tests it: a machine that does not answer is refused with its own words and the switch does not happen, so the person stays on the machine they were already using. `local` is not tested — it is this process.
