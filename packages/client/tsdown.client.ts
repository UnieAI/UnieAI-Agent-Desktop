 ...
 */
export const INLINE_SAFE = /^@unieai\/uad-(host-apiproxy|file-reference|session|llm|tools|brand|upstream-names|studio-kb-sources)(\/|$)/
...
const GENERATED_REMOTE = /^@unieai\/uad-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @unieai value import is a build error — a
      ...
      resolveId(source: string) {
        if (!source.startsWith('@unieai/')) return null
