...
import type { Session, SessionEvent, SessionHeader, SessionId } from '@unieai/uad-session'
import type { SessionPersistence } from '@unieai/uad-session-persistence'
import type { SessionProjectionRegistry } from '@unieai/uad-session-projection'
import type { SessionProjectionCache } from '@unieai/uad-session-projection-cache'
...
async function prepareListing(
  ...
  if (projections === undefined) {
    throw new SubagentError(
      'listing subagents requires the sessionProjections registry (load @unieai/uad-session-projection)',
  ...
  if (sessions === undefined) {
    throw new SubagentError(
      'listing subagents requires the session store (load @unieai/uad-session)',
