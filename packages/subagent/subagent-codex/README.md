...
The standalone composition below shows the complete explicit capability. A Profile based on `@unieai/uad-base` keeps its existing Job rows, adds the product provider and tool rows, and does not mount duplicate Job services.

```yaml
- id: subagent-codex-safe
  name: '@unieai/uad-subagent-codex'
  config:
...
```yaml
- id: jobs
  name: '@unieai/uad-jobs-local'

- id: tool-jobs
  name: '@unieai/uad-tool-jobs'
