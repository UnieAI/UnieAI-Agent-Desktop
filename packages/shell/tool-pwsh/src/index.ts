 ...
 * `@unieai/uad-pwsh-local`) backs `ctx.shell`; the tool contract is
 ...
 * exit-status parse from `@unieai/uad-shell`.
 *
 * @module @unieai/uad-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@unieai/cordis'
import z from '@unieai/schemastery'
import { defineTool, TOOL_ABORTED } from '@unieai/uad-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@unieai/uad-tools'
...
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@unieai/uad-sandbox'
import type { SandboxPolicyService } from '@unieai/uad-sandbox-policy'
import type { ShellRunResult } from '@unieai/uad-shell'
import { parseExitStatus } from '@unieai/uad-shell'
import { processOutcome } from './background.ts'
import { renderPwshProcessRead, renderPwshResult } from './render.ts'
import type { RenderablePwshResult } from './render.ts'

declare module '@unieai/uad-jobs' {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: PwshToolArgs, exec) {
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @unieai/uad-jobs and @unieai/uad-tool-jobs')
        }
