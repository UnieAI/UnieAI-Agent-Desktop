...
| Package | Role |
|---|---|
| `@unieai/uad-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@unieai/uad-bash-local` | Service Provider: local subprocesses |
| `@unieai/uad-bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@unieai/uad-tool-bash` | the model-facing tool schemas over `ctx.shell` |
