...
/**
 * Keyless built-artifact smoke: plain Node imports `@unieai/uad-lsp` and
 * `@unieai/uad-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@unieai/cordis')
      const { default: Lsp } = await import('@unieai/uad-lsp')
      const LspLocal = await import('@unieai/uad-lsp-stdio')
      const { default: LocalFileSystem } = await import('@unieai/uad-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@unieai/uad-subprocess-local')
