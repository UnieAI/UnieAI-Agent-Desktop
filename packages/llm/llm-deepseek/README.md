...
The catalog and the credential are separate questions, and this plugin answers both. `ctx.llm.listModels('deepseek-official')` reports the shipped three-model catalog whether or not a key was ever stored — it is a constant, not a probe — so the adapter also implements `credentialReady`, which answers whether a request could authenticate *right now* and does so through `credentials.describe`, value-free: the readiness probe never touches the secret it stands next to. Without a credential seam mounted the launching environment is the whole credential plane and is checked instead, exactly as `resolveApiKey` ranks them. That is what lets a consumer building a menu list only models a request could actually reach, while first-run onboarding keeps working: the route stays registered, `listModels` keeps answering, and the moment a key is stored the readiness answer flips with no restart and no re-registration.
...
DeepSeek request identity is separate from app attribution. After credential resolution, every provider request carries `x-deepseek-harness-user-id` with the stable anonymous id from [`@unieai/uad-anonymous-user-id`](../../identity/anonymous-user-id/README.md); a request carrying `GenerateOptions.sessionId` also sends that exact value as `x-deepseek-harness-session-id`, while a direct call without a session omits the session header. Both headers go to the resolved `baseURL`, including a configured gateway, and remain outside the request body and model-visible content.
