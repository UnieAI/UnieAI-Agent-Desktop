...
import LlmRuntime, { createMessage, createUserMessage } from '@unieai/uad-llm'
...
async function loadComposition(): Promise<{ ctx: Context; settingsPath: string }> {
  ...
  await writeFile(configPath, [
    ...
    '- id: settings',
    "  name: '@unieai/uad-settings-file'",
    '  config:',
    `    path: ${JSON.stringify(settingsPath)}`,
    '    debounceMs: 10',
    '- id: credentials',
    "  name: '@unieai/uad-credentials-local'",
    '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@unieai/uad-settings-file', FileSettingsProvider],
    ['@unieai/uad-credentials-local', LocalCredentialProvider],
    ['@unieai/uad-llm-pi-ai', LlmPiAi],
  ])
  ctx.loader.internal = {
    version: 'v2',
