...
The adapter also answers `ctx.llm.credentialReady(provider)`, which is a different question from "does this route have models" and is asked without reading the credential. A profile naming no `apiKeyEnv` is reported READY: that is how a deployment records provider-native authentication — the Bedrock credential chain, Vertex ADC, a stored login — and calling it unconfigured would hide exactly the routes that need no key. A profile that does name a reference is judged by `credentials.describe`, which reports what `resolve` would find without disclosing it, and without a mounted seam by the referenced environment variable; only that case can answer `false`. A route this adapter does not own answers `undefined` rather than `false`, so one adapter family cannot hide another's models.
