...
import TypertRegistry from '@unieai/uad-typert-registry'
import type { TypertContribution } from '@unieai/uad-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@unieai/uad-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    ...
    const artifact = new FaceModelEmitter(host).emit('@unieai/uad-tools')
    ...
    const record = ctx.typert.getPackage('@unieai/uad-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@unieai/uad-tools', 'host')).toBeUndefined()
  })
})
