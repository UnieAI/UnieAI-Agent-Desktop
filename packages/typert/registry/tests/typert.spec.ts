...
} from '@unieai/uad-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@unieai/uad-typert-protocol'
import { apply as applyClientRegistry, inject as clientRegistryInject } from '../src/client/index.ts'

declare module '@unieai/uad-typert-protocol' {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@unieai/uad-tools', 'ToolInput')).toBe('@unieai/uad-tools#ToolInput')
    expect(typertPackageKey('@unieai/uad-tools', 'host')).toBe('@unieai/uad-tools#host')
    expect(ctx.typert.get('@unieai/uad-tools#ToolInput')).toMatchObject({
      package: '@unieai/uad-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@unieai/uad-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@unieai/uad-tools', 'host')).toMatchObject({
      key: '@unieai/uad-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@unieai/uad-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@unieai/uad-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@unieai/uad-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@unieai/uad-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@unieai/uad-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@unieai/uad-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@unieai/uad-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@unieai/uad-tools#ToolInput')).not.toBe(projected)
  })
