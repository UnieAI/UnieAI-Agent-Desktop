...
- **One machine per mount.** The alias is configuration, so every mount of this provider runs its commands on one machine. Several machines at once is a caller's composition — [`execution-router`](../../machines/execution-router/README.md) builds one runtime per machine and picks between them — and that caller constructs this class directly, which is why the machine book can be handed to the constructor as well as injected: a provider built with `new` sits on a context that declares nothing, and reading `ctx.ssh` there is refused rather than answered.
