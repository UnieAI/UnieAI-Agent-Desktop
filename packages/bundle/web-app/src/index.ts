/**
 * @unieai/uad-web-app — the browser-surface bundle's runtime glue plugin
...
import { addHarnessSourceSection } from '@unieai/uad-app-boot'
import * as FrontendStatic from '@unieai/uad-host-frontend-static'
import { launchEnvironmentOf } from '@unieai/uad-launch-environment'
import { scrubbedParentEnv } from '@unieai/uad-subprocess'
import type {} from '@unieai/cordis-plugin-loader'
import type {} from '@unieai/uad-host-webserver'
import type {} from '@unieai/uad-system-prompt'
import type {} from '@unieai/uad-shell-env'
...
/** Model-visible orientation and acceptance boundary for sessions created through `rabi web`. */
function webSurfacePrompt(webUrl: string): string {
  ...
  return `You are interacting with the user through the DeepSeek Harness Web GUI at ${webUrl}. `
    ...
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only rabi web injects window.__DSH_BOOT__. '
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('@unieai/uad-web-frontend/dist/index.html')
  } catch {
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (config.printUrl || handoffBrowser) {
    ...
    const announceReady = (): void => {
      ...
      if (config.printUrl) {
        console.log(`rabi web: ${webUrl}${lanCandidate === undefined ? '' : ` (LAN: http://${lanCandidate}:${String(port)})`}`)
      }
      if (handoffBrowser) {
        console.log('rabi web: opening the default browser; pass --no-open to disable')
