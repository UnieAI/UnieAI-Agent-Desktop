...
import AgentRegistry, { agentEvents, Inbox } from '@unieai/uad-agent'
import type { Agent } from '@unieai/uad-agent'
import { createUserMessage, HarnessError } from '@unieai/uad-llm'
import SessionStore, { Session, SessionId, type UserMessage } from '@unieai/uad-session'
import GoalService, {
  GoalError,
  GoalId,
  decodeGoalChange,
  foldGoal,
} from '@unieai/uad-goal'
import type { GoalChangeMeta, GoalRef, GoalSnapshotChangeMeta } from '@unieai/uad-goal'
...
function stubAgent(rawId: string, seed?: readonly import('@unieai/uad-session').SessionEvent[]): StubAgent {
