...
import * as StorageDomain from '@unieai/uad-storage-domain'
import * as StorageJson from '@unieai/uad-storage-json'
import { remoteMethods } from '@unieai/uad-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@unieai/uad-session-persistence-jsonl', JsonlSessionPersistence],
    ['@unieai/uad-storage', Storage],
    ['@unieai/uad-storage-json', StorageJson],
    ['@unieai/uad-storage-domain', StorageDomain],
    ['@unieai/uad-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@unieai/uad-storage'",
      "- name: '@unieai/uad-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@unieai/uad-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@unieai/uad-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
