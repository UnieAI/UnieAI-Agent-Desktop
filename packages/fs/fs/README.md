...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@unieai/uad-tool-fs` | model-facing `read`/`write`/`edit` schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `@unieai/uad-fs-observation-policy` | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `@unieai/uad-fs` (this) | `ctx.fs`: execution-world paths, text IO, and atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `@unieai/uad-fs-local` | the host-filesystem implementation |
...
| `createDirectory?(parent, name, signal?)` | **The one optional method on this seam.** Create one directory under an existing parent; non-recursive, because the parent is a directory a caller is already looking at and `-p` would invent a chain nobody asked for. The name must be one segment — a separator, `.` or `..` is refused with `FS_NOT_DIRECTORY`. Something already there throws `FS_ALREADY_EXISTS`, a missing parent `FS_NOT_FOUND`. Optional because every other method here is something a provider must do to be a filesystem at all, while this one exists for the surfaces that let a person make a folder while choosing one; a caller checks for the method and says what it cannot do rather than failing at the attempt. |
...
- **Twelve primitives and one optional thirteenth** — no delete, rename/move, copy, or watch; directory creation is the optional one and every other mutation is a file; `listDir` is single-level, with recursion, globbing, pagination, and search out of scope per [the directory-listing Agent Note](../../../.agents/notes/archived/architecture/2026-07-03-filesystem-directory-listing-seam.md).
