 ...
 *
 * @module @unieai/uad-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@unieai/uad-output-retention'
import type { RetainedItems } from '@unieai/uad-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@unieai/uad-subprocess'
import type { SaveTextSpill, SpillRef } from '@unieai/uad-spill'
import type { ToolExecution } from '@unieai/uad-tools'
 ...
 * `@unieai/uad-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@unieai/uad-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@unieai/uad-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@unieai/uad-tool-fs-search/presentation} `grepSearchMeta`)
