...
await ctx.plugin(LocalSubprocessRuntime)                     // @unieai/uad-subprocess-local
...
await ctx.plugin(LocalSpillStore)                           // @unieai/uad-spill-local
```
...
| `timeoutMs` | `30000` | Cooperative tool-call budget attached to both tool definitions, enforced by `@unieai/uad-tool-call-timeout-policy` through `exec.signal`; the subprocess seam's terminate escalation is the hard kill. |
...
The glob description states the configured over-cap ordering. The generated [`glob` and `grep` schemas](../../../docs/tool-catalog.md#unieaiuad-tool-fs-search) use `sampleOverCapGlobResults: true`; the tools are registered unconditionally.
