...
```
@unieai/cordis-plugin-timer  timer service (writes nothing to stdout)
@unieai/uad-llm              abstract LLM service + content-block vocabulary
@unieai/uad-session          event-sourced session log + store
@unieai/uad-session-title    log-backed title service + deterministic fallback
@unieai/uad-system-prompt    prompt-section + tool-schema assembly
@unieai/uad-tools            registry + guarded pre/around/post/final-result pipeline
@unieai/uad-skill            skill provider registry
@unieai/uad-skill-filesystem      local filesystem skill provider
@unieai/uad-agent            agent registry + initiator scope + agent/* events
@unieai/uad-goal             optional persisted same-session goal domain
@unieai/uad-tool-goal        optional model-facing goal controls
@unieai/uad-goal-round-driver     optional same-session goal-round driver
@unieai/uad-llm-retry        provider-routed request retry policy
@unieai/uad-jobs-local      generic background-job registry
@unieai/uad-invariants       configurable invariant registry service
@unieai/uad-session/invariant
@unieai/uad-agent/invariant
@unieai/uad-scope/invariant
@unieai/uad-agent-loop/invariant
                                  package-owned relational checks
@unieai/uad-tool-bash        the model-facing bash schema (unless toolBash=false)
@unieai/uad-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@unieai/uad-tool-skill       session-prefix skill catalog + model-facing loader schema
@unieai/uad-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@unieai/uad-agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@unieai/uad-agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@unieai/uad-'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.zh.md)。
