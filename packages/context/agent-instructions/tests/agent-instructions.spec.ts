...
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@unieai/uad-llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@unieai/uad-session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@unieai/uad-agent'
import AgentLoop from '@unieai/uad-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@unieai/uad-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import type {
  ToolExecution,
  ToolExecutionToken,
} from '@unieai/uad-tools'
import * as ToolFs from '@unieai/uad-tool-fs'
import {
  discoverBaselineInstructionFiles,
  loadBaselineInstructions,
  renderWorkspaceContext,
} from '@unieai/uad-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    try {
      ...
      const isolated = await import('@unieai/uad-agent-instructions')
