...
- `ctx.agentPresets.standingService(name, id?): Promise<Context[name] | undefined>` 常驻挂载**发布**的某一个服务，服务对象同样是那个没有 agent 的读取方。`standingKeyFor` 回答的是注册在哪个 scope 里解析，这一条回答的是：由 realm 发布的那个服务，实际是哪个实例在服务它们。一个自己挂载注册表的 preset——比如技能注册表——会让那个实例对宿主 context 不可见；于是一个读 `ctx.skills` 的宿主界面看到的是一个空注册表、报出一份空目录，而每一轮对话看到的却是满的。挂载没有发布这个服务时回 `undefined`。
...
行的**包名**从宿主组装解析，而非从 preset 目录解析。Loader 通常按 entry 所属树的 `baseUrl` 解析，而对 preset 而言那就是组装文件所在之处；本地创作的 preset 位于用户主目录之下，Node 向上查找 `node_modules` 永远够不到 harness，因此每一个 `@unieai/uad-*` 行都会导入失败。挂载在插入子树之前先记录宿主的基址，并把裸标识符送往那里。
