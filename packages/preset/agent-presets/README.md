...
- `ctx.agentPresets.standingService(name, id?): Promise<Context[name] | undefined>` One service the standing mount PUBLISHES, for the same agent-less reader. `standingKeyFor` answers which scope registrations resolve in; this answers which instance of a realm-published service serves them. A preset that mounts its own registry — the skill registry, say — keeps that instance invisible to host contexts, so a host surface reading `ctx.skills` would see an empty registry and report an empty catalogue while every turn saw a full one. `undefined` when the mount publishes no such service.
