...
This package implements one skill source. It scans local project, custom, and user skill roots, parses `SKILL.md` or flat Markdown skill files, and registers the provider on `ctx.skills`. The registry remains in `@unieai/uad-skill`; the durable session catalogs and model-facing loader tool remain in `@unieai/uad-tool-skill`.
...
| `dshHome` | `$DSH_HOME` or `~/.dsh` | DeepSeek Harness config root resolved by [`@unieai/uad-home-paths`](../../util/home-paths/README.md); scans `skills` under this directory. |
