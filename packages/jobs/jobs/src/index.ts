 ...
 * process-local registry lives in `@unieai/uad-jobs-local`.
 * @module @unieai/uad-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@unieai/uad-jobs is the abstract job registry seam; load an implementation such as @unieai/uad-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
