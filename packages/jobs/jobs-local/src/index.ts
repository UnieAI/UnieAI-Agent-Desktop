 ...
 * @module @unieai/uad-jobs-local
 */

import { Context } from '@unieai/cordis'
import z from '@unieai/schemastery'
import type { Agent } from '@unieai/uad-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@unieai/uad-scope'
import type { ScopeLayer } from '@unieai/uad-scope'
import { deadline, timeoutOf } from '@unieai/uad-timeout'
import { JobRegistry, JobId } from '@unieai/uad-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@unieai/uad-jobs'
 ...
 * `@unieai/uad-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @unieai/uad-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @unieai/uad-agent)')
    }
