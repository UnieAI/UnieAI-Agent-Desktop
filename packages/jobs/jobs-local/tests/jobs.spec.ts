...
import { bindScopeParent, createScope, scopeOf } from '@unieai/uad-scope'
import type { ScopeKey } from '@unieai/uad-scope'
import { JobId } from '@unieai/uad-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@unieai/uad-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @unieai/uad-tool-jobs in its composition)')
  })
