 ...
 * @module @unieai/uad-sandbox-local
 */
...
} from '@unieai/node-addon-landlock-run'
import { Context } from '@unieai/cordis'
import z from '@unieai/schemastery'
import { assertNever } from '@unieai/uad-llm'
import { SandboxProvider, SandboxUnavailableError } from '@unieai/uad-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@unieai/uad-sandbox'
import type { SessionId } from '@unieai/uad-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@unieai/uad-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@unieai/uad-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@unieai/uad-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@unieai/uad-sandbox-windows-acl/src/runner.ts'))
