...
- `ctx.cmdlineArgs` — the invocation's inner arguments. `get()` is the whole interface, and it returns a snapshot: `rabi --profile tui --resume abc` yields `['--resume', 'abc']`.
...
```yaml
- id: web-startup
  name: '@unieai/uad-web-app/startup'
```
