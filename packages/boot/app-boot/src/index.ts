...
import { Context, type FiberState } from '@unieai/cordis'
import Loader, { type Entry, type EntryOptions } from '@unieai/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@unieai/cordis-plugin-include'
import Group from '@unieai/cordis-plugin-group'
import { dshHomePath, resolveDshHome } from '@unieai/uad-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@unieai/uad-launch-environment'
import type {} from '@unieai/cordis-plugin-hmr'
 ...
 * entries (`@unieai/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@unieai/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@unieai/cordis-plugin-group`
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The Rabi implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend DSH itself.`,
  })
}
