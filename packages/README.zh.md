...
npm scope 为 `@unieai/uad-*`；Cordis `Service` 子类和函数插件通过 `ctx.effect()`、`ctx.on()` 或 `ctx.waterfall()` 注册。规则见[包](AGENTS.md)与[根规则](../AGENTS.md#conventions)。

## 层级结构

包按组置于 `packages/<group>/<pkg>/`；包名仍为 `@unieai/uad-<pkg>`。**组 README 负责包／ctx 键映射。**
...
| [`ssh/`](ssh/README.zh.md) | 透過 OpenSSH 客戶端連到遠端機器 | 提供方 |
| [`machines/`](machines/README.zh.md) | 工作發生在哪裡：機器清單與路由式執行世界 | 提供方 |
...
| [`bundle/`](bundle/README.zh.md) | 可安装的 `rabi --profile` 补丁层 | 产品：稳定 API |
