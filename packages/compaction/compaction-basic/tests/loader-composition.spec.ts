...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@unieai/uad-llm', LlmRuntime],
    ['@unieai/uad-session', SessionStore],
    ['@unieai/uad-token-meter', TokenMeter],
    ['@unieai/uad-compaction-tool-result-pruner', ToolResultPruner],
    ['@unieai/uad-compaction-basic', BasicCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-basic YAML order', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@unieai/uad-token-meter'",
      "- name: '@unieai/uad-compaction-tool-result-pruner'",
      '  config:',
      '    thresholdChars: 100',
      '    headChars: 20',
      '    tailChars: 10',
      "- name: '@unieai/uad-compaction-basic'",
      '  config:',
      '    thresholdRatio: 0.5',
      '    retainRatio: 0.125',
      '    auto: false',
