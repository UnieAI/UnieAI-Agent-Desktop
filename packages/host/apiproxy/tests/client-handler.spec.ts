...
import type { ApiProxy, GoalRef, HostFrame, MuxFrame, RpcMessage, RpcRequest, RpcResponse } from '@unieai/uad-host-apiproxy'
import { InProcessApiClient, RpcId, toFetchHandler } from '@unieai/uad-host-apiproxy'
...
function scriptedApi(overrides: {
  ...
  return {
    ...
    host: {
      ...
      listWorkspaceEntries: r => ok(r, { root: '/t', path: '/t', entries: [], truncated: false }),
      readWorkspaceFile: r => ok(r, { root: '/t', path: '/t/a', size: 0, text: '' }),
    ...
    skills: {
      ...
      install: r => ok(r, { name: r.payload.slug, path: `/skills/${r.payload.slug}/SKILL.md`, replaced: false }),
      ...overrides.skills,
    },
    agentPresets: {
