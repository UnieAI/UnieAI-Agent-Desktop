...
import type { SessionLineageNode } from '@unieai/uad-session-query'
import type { SessionRawArtifact } from '@unieai/uad-session-persistence'
import ApiProxyService, { createApiProxy, toFetchHandler } from '@unieai/uad-host-apiproxy'
...
describe('session export compression config', () => {
  it('defaults to level 6 and rejects values outside the integer 0-9 range', () => {
    expect(ApiProxyService.Config({})).toEqual({
      ...
      workspaceListingMaxEntries: 1000,
      workspaceFileMaxBytes: 1000000,
    })
    expect(ApiProxyService.Config({ sessionExportCompressionLevel: 0 }))
      .toEqual({
        sessionExportCompressionLevel: 0, coldBlankProbeMaxBytes: 1024,
        workspaceListingMaxEntries: 1000, workspaceFileMaxBytes: 1000000,
      })
