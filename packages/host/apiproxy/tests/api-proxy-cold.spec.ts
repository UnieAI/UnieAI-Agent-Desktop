...
import { TypertLookupFailure } from '@unieai/uad-typert-protocol'
import TypertRegistry from '@unieai/uad-typert-registry'
import { createUserMessage, MessageId } from '@unieai/uad-llm'
import type { Agent } from '@unieai/uad-agent'
import UserQuestionService from '@unieai/uad-user-questions'
import type { SessionEvent, SessionHeader, SessionId } from '@unieai/uad-session'
import {
  PersistenceCoordinator,
  SessionPersistenceRevision,
  type PersistenceBackend,
  type StoredPrefix,
} from '@unieai/uad-session-persistence'
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@unieai/uad-session').Session
