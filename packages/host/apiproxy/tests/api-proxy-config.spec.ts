...
import LlmRuntime, { LlmAdapter } from '@unieai/uad-llm'
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@unieai/uad-llm'
import { SettingsProvider, settingsNamespace } from '@unieai/uad-settings'
import type { SettingsNamespace } from '@unieai/uad-settings'
...
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@unieai/uad-agent-default-model'
...
describe('settings domain', () => {
  it('reports an actionable error when no settings provider is mounted', async () => {
    ...
    expect(error.message).toContain('uad-settings-file')
  })
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('uad-credentials-local')
  })
